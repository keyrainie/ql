        ...
        private static object Convert(Type sourceType, Type targetType, object source, bool propertyNameIgnoreCase, string columnName, string propertyName, List<string> ignoreTargetProperties)
        {
                ...
                )
            {
                ...
                foreach (object x in iterator)
                {
                    ...
                    object target_key = Convert(source_key_type, target_key_type, source_key, propertyNameIgnoreCase, columnName, propertyName, ignoreTargetProperties);
                    object target_value = Convert(source_value_type, target_value_type, source_value, propertyNameIgnoreCase, columnName, propertyName, ignoreTargetProperties);
                ...
                )
            {
                ...
                foreach (object x in iterator)
                {
                    Invoker.MethodInvoke(list, "Add", Convert(s_eType, listType, x, propertyNameIgnoreCase, columnName, propertyName, ignoreTargetProperties));
                }
                return list;
            }
