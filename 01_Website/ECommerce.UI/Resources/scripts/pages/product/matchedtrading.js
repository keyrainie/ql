var matchedtrading = {
    ...
    checkTimeLimitBeforePublish: function (postTimeLimit) {
        ...
        if (nowMinitePoint - matchedtrading.preTimePoint < postTimeLimit) {
            return false;
        }
        return true;
    },
    resetPublishLimitTime: function () {
        var nowTime = new Date();
        var nowMinitePoint = nowTime.getHours() * 3600 + nowTime.getMinutes() * 60 + nowTime.getSeconds();
        matchedtrading.preTimePoint = nowMinitePoint;
    },
    //发表撮合交易
    creatematchedtrading: function (obj) {
        matchedtrading.CheckLogin();
        if (!matchedtrading.checkTimeLimitBeforePublish(60)) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data <= 0) {
                    ...
                    matchedtrading.resetPublishLimitTime();
                }
            },
            complete: function () {
    ...
    CreateconsultReply: function (obj) {
        ...
        $.ajax({
            ...
            complete: function () {
                $btn.removeClass("btn_disconsult").addClass("btn_consult").attr("onclick", "matchedtrading.createconsultReply(this);");
            }
        });
