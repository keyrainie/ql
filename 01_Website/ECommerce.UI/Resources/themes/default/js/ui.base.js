...
var bindSelect=function(_obj){
	...
	obj.mouseenter(function(){		
		...
		UI.laterEvent(function(){			
			...
			var pli = me.parents("li");
			pli.css({zIndex:pli.attr("data-default-zindex")!=undefined ? pli.attr("data-default-zindex") : ""});
	...
	obj.find("dt").click(function(){
		...
		curselect.parents("li").css({zIndex:2});
		//Start: Options Width & Height Fix		
		curselect.find("ul").css("width","");	
		curselect.find("dd").css({width:"",heigth:""});
		...
		if(ie6){
			max_w = (curselect.attr("data-max-width")!=undefined) ? curselect.attr("data-max-width") : null;
			if(!max_w){
				max_w = curselect.css("maxWidth") ? curselect.css("maxWidth") : curselect.css("width");	
...
$(function () {
	...
	$(".price_old").each(function(){
		$(this).html("<s style='display:inline-block;margin-left:-11px;width:10px;'>&nbsp;&nbsp;&nbsp;</s>"+$(this).text());	
	});
	...
	$(".menunavi").find(">li dt").hover(function(){
		...
		UI.laterEvent(function(){
			...
			if(!$this[0].ddtop){
				...
				var hasPromo = true;
				var promotimgWidth;
				if($(".promotimg img",dd).length==0 && $(".promotext",dd).length==0){
					hasPromo = false;
					promotimgWidth = -20;
				}
				else{
					promotimgWidth = 178;				
					$(".ddwrap",dd).addClass("ddwrap_bg_col1_hasProm");	
					$(".relativeinfo",dd).show();
				}
				if(cateWrap.height()>331){
					...
					if(hasPromo==true){
						promotimgWidth = 178;
						$(".ddwrap",dd).addClass("ddwrap_bg_col2_hasProm");
					}
					else{
						promotimgWidth = -20;
						$(".ddwrap",dd).addClass("ddwrap_bg_col2");	
					}				
				}
				if(ie6 && !hasPromo){
					cateWrap.css("width",cateWrap.width()-20);
				}
