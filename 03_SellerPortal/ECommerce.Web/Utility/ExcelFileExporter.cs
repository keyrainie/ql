...
namespace ECommerce.Web.Utility
{
    public class ExcelFileExporter : IFileExport
    {
        ...
        protected XlsDocument m_xlsDocument = new XlsDocument();
        protected Dictionary<string, XF> m_xlsXFs = new Dictionary<string, XF>();
        ...
        public byte[] CreateFile(List<DataTable> data, List<List<ColumnData>> columnList, List<TextInfo> textInfoList, out string fileName, string FileTitle)
        {
            int index = 0;
            ...
            foreach (DataTable table in data)
            {
                ...
                {
                    //string msg = string.Format(ECCentral.Service.Utility.Resources.ErrorMsg.DataExport_DataTableRowCountExceedLimit, MaxDataTableRowCountLimit);
                ...
                if (table != null)
                {
                    BuildSheet(worksheet, index, table, GetSetting(columnList, index), textInfoList);
                }
                index++;
            }
            fileName = SetFileName(data, null);
        ...
        protected virtual void BuildSheet(Worksheet worksheet, int sheetIndex, DataTable data,
            List<ColumnData> columnSetting, List<TextInfo> textInfoList)
        {
            ...
            XF sxf = GetDataHeaderXF(sheetIndex, 1, HorizontalAlignments.Left, VerticalAlignments.Centered, false);
            ...
            for (int r = 1; r <= 4; r++)
            {
                worksheet.Cells.Merge(r, r, 1, columnSetting.Count);
            }

            worksheet.Cells.Add(1, 1, "报表名字：" + FileTitle, sxf);
            worksheet.Cells.Add(2, 1, "公司名称：浙江泰隆商业银行电商平台", sxf);
            worksheet.Cells.Add(3, 1, "制定时间：" + DateTime.Now.ToString("G"), sxf);
            worksheet.Cells.Add(4, 1, "打印时间：" + DateTime.Now.ToString("G"), sxf);

            if (textInfoList != null && textInfoList.Count > 0)
            {
                headerRowIndex = textInfoList.Count + headerRowIndex;

                for (int r = 1; r <= textInfoList.Count; r++)
                {
                    worksheet.Cells.Merge(r + 4, r + 4, 1, columnSetting.Count);

                    worksheet.Cells.Add(r + 4, 1, textInfoList[r - 1].Title + "  " + textInfoList[r - 1].Memo, sxf);
                }
            }
        ...
        {
            Type type = Type.GetType("ECommerce.Utility.BusinessException, ECommerce.Utility");
