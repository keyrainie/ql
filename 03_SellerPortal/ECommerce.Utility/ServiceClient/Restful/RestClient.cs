...
namespace ECommerce.Utility
{
    public class RestClient
    {
        ...
        {
            return (T)Invoke(typeof(T), url, data, method, format, requestEncoding, responseEncoding);
        }

        public object Invoke(Type returnType, string url, object data, HttpMethod method, RequestFormat format,
            Encoding requestEncoding, Encoding responseEncoding)
        {
            if (format == RequestFormat.Raw && returnType != typeof(string) && returnType != typeof(object))
            {
            ...
            OnAfterResponse(rsp);
            return DeserializeResponseData(rsp, format, returnType);
        }
        ...
        public object Invoke(Type returnType, string url, object data = null, HttpMethod method = HttpMethod.Get, RequestFormat format = RequestFormat.Json)
        {
            return Invoke(returnType, url, data, method, format, Encoding.UTF8, Encoding.UTF8);
        }
