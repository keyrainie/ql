...
{
    public class AsynQueueManager<T>
    {
        ...
        public AsynQueueManager(IQueue<T> queue, Action<List<T>> queueItemHandler = null, int itemCountPerBatchToHandle = 256,
            int dequeueThreadAmountMaxLimit = 5, int flushIntervalSeconds = 60, Action<Exception> exceptionHandler = null)
        {
            m_Queue = queue;
            ...
            m_FlushInterval = flushIntervalSeconds * 100;
        }
        ...
        private void StartNewMessageWatcher()
        {
            ...
            Thread thread = new Thread(o =>
            {
                ...
                {
                    times++;
                    if (1 == Interlocked.Exchange(ref m_HasNewMessage, 0) || times >= m_FlushInterval) // 默认为60 seconds
                    {
                        ReadQueue();
                        times = 0;
                    }
        ...
        private void HandleMessages(List<T> list)
        {
