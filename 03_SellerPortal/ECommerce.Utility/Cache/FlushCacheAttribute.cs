...
namespace ECommerce.Utility
{
    ...
    public class FlushCacheAttribute : OnMethodBoundaryAspect
    {
        ...
        public override bool CompileTimeValidate(MethodBase method)
        {
            ...
            if (m_Type == null)
            {
                Message.Write(MessageLocation.Unknown, SeverityType.Error, "ER0008", "The type in FlushCacheAttribute is null with the method : '" + method.DeclaringType.FullName + "." + method.Name + "'.");
                return false;
            }
            MethodInfo cacheMethod = m_Type.GetMethod(m_MethodName);
            if (cacheMethod == null)
            {
                Message.Write(MessageLocation.Unknown, SeverityType.Error, "ER0009", "The method '" + m_MethodName + "' of type '" + m_Type.FullName + "' in FlushCacheAttribute is not found with the method : '" + method.DeclaringType.FullName + "." + method.Name + "'.");
                return false;
            }
            ...
            {
                Message.Write(MessageLocation.Unknown, SeverityType.Error, "ER0010", "The method '" + m_MethodName + "' of type '" + m_Type.FullName + "' in FlushCacheAttribute is not marked by CachingAttribute with the method : '" + method.DeclaringType.FullName + "." + method.Name + "'.");
                return false;
            }
            ...
            if (m_Keys != null && m_Keys.Count > 0)
            {
                ...
                foreach (ParamCacheKey key in m_Keys)
                {
                    ...
                    if (param == null)
                    {
                        Message.Write(MessageLocation.Unknown, SeverityType.Error, "ER0006", "There is not a parameter named '" + key.ParamName + "' with the method '" + method.DeclaringType.FullName + "." + method.Name + "'.");
                        return false;
                    }
                    if (key.ParamProperty != null && key.ParamProperty.Length > 0)
                    {
                        ...
                        for (int i = 0; i < key.ParamProperty.Length; i++)
                        {
                            ...
                            {
                                Message.Write(MessageLocation.Unknown, SeverityType.Error, "ER0007", "Error with the Keys : there is no property '" + key.ParamProperty[i] + "' can be read in type '" + tmpType.AssemblyQualifiedName + "'. Method : '" + method.DeclaringType.FullName + "." + method.Name + "'");
                                return false;
                            }
