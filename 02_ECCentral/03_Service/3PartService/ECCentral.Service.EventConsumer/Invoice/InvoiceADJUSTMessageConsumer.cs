...
{
    public class InvoiceADJUSTMessageConsumer : IConsumer<InvoiceADJUSTMessage>
    {
        public void HandleEvent(InvoiceADJUSTMessage eventMessage)
        {
        ...
        private void SendMessage(InvoiceADJUSTMessage eventMessage)
        {
        ...
        private void CheckMessage(InvoiceADJUSTMessage eventMessage)
        {
            ...
            if (eventMessage.Body == null || eventMessage.Body.Count == 0)
            {
                throw new ArgumentException("message.Body can not be null or empty.");
            }
            if (StringUtility.IsNullOrEmpty(eventMessage.Body[0].MSMQAddress))
            {
                throw new ApplicationException("MSMQ address not found. WarehouseNumber: [" + eventMessage.Body[0].StockID + "]");
            }
        }
