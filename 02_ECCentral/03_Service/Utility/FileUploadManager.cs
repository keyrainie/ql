...
namespace ECCentral.Service.Utility
{
    public static class FileUploadManager
    {
        ...
        public static string GetFilePhysicalFullPath(string fileIdentity)
        {
        ...
        public static string UploadFileAndGetUrl(string fileName, byte[] fileData)
        {
            return DataFileExporter.BringUrl(fileName, fileData);
        }
    }
    ...
    public class FileUploadHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            string upLoadBaseFolder = context.Request.QueryString["UpLoadBaseFolder"];
            if (fileIdentity == null || fileIdentity.Trim().Length <= 0)
            {
            ...
            if (!string.IsNullOrEmpty(upLoadBaseFolder))
                upLoadBaseFolder = Encoding.UTF8.GetString(Convert.FromBase64String(upLoadBaseFolder));
            ...
            if (!string.IsNullOrEmpty(upLoadBaseFolder))
            {
                filePath = Path.Combine(upLoadBaseFolder, fileIdentity);
            }
            else
            {
                filePath = Path.Combine(FileUploadManager.BaseFolder, fileIdentity);
            }
