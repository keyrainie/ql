    ...
    public class EventConsumerRegister : IStartup
    {
        public void Start()
        {
            ...
            foreach (XmlNode ev in eventList)
            {
                ...
                string persistStr = GetNodeAttribute(ev, "persist");
                if (persistStr == null ||
                    (persistStr.Trim().ToUpper() != "FALSE" && persistStr.Trim().ToUpper() != "N" && persistStr.Trim() != "0"))
                {
                    Type tmpType = typeof(ESBConsumer<>).MakeGenericType(eventType);
                    object tobj = Activator.CreateInstance(tmpType);
                    method.Invoke(null, new object[] { tobj });
                }
                else
                {
                    Type tmpType = typeof(DummyConsumer<>).MakeGenericType(eventType);
                    object tobj = Activator.CreateInstance(tmpType);
                    method.Invoke(null, new object[] { tobj });
                }
