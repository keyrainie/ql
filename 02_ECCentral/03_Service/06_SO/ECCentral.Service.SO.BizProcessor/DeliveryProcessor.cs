...
namespace ECCentral.Service.SO.BizProcessor
{
    ...
    public class DeliveryIteration
    {
        ...
        public DeliveryIteration(DateTime startTime, int deliveryType, List<Holiday> holidays, List<TimeSpan> timePoints, int? prepareSections)
            : this(startTime, deliveryType, holidays, timePoints, 0, prepareSections)
        {
        }

        public DeliveryIteration(DateTime startTime, int deliveryType, List<Holiday> holidays, List<TimeSpan> timePoints, int IntervalDays)
            : this(startTime, deliveryType, holidays, timePoints, IntervalDays, null)
        { }

        public DeliveryIteration(DateTime startTime, int deliveryType, List<Holiday> holidays, List<TimeSpan> timePoints, int IntervalDays, int? prepareSections)
        {
        ...
        public void GetDeliveryType()
        {
            ...
            {
                //检查节假日
                var hd = Holidays.Find(x => x.HolidayDate == StartTime.Date);
            ...
            else if (OriginalDeliveryType == 2)
            {
                ...
                //检查节假日
                var hd = Holidays.Find(x => x.HolidayDate.Date == StartTime.Date);
                if (hd != null)
