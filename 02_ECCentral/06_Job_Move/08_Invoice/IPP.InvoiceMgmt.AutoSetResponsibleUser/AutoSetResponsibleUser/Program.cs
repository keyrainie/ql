...
using System.Text;
using AutoSetResponsibleUser.Components;
using AutoSetResponsibleUser.BusinessLayer;
using AutoSetResponsibleUser.Common;
using Newegg.Oversea.Framework.JobConsole.Client;

namespace AutoSetResponsibleUser
{
    public class Program : IJobAction
    {
        ...
        public void Run(JobContext context)
        {
            ...
            AutoSetResponsiblerUserBP business = null;

            try
            {
                business = new AutoSetResponsiblerUserBP();
                business.DisplayMessage += WriteLog;

                business.CreateTrackingInfo();
            }
            catch (Exception ex)
            {
                WriteLog(ex);
