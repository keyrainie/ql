...
namespace IPP.OrderMgmt.JobV31.Biz.FPCheck
{
    public class SOFPCheckBP
    {
        ...
        {
            bool result = false;
            if (FPCheckMasterList.Count > 0 && FPCheckMasterList.Find(x => { return (x.CheckType == "CC" && x.Status == 0); }) != null)
            {
                result = true;
            }
            return result;
        ...
        private static bool GetCheckChongFuFlag()
        {
            //判断是否进行炒货检测          
            bool result = false;
            if (FPCheckMasterList.Count > 0 && FPCheckMasterList.Find(x => { return (x.CheckType == "CF" && x.Status == 0); }) != null)
            {
                result = true;
            }
            return result;
