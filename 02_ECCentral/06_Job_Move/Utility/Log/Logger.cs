    ...
    public static class Logger
    {
        private static void WriteEventLog(string source, string content, EventLogEntryType type)
        {
            try
            {
                ...
                {
                    EventLog.CreateEventSource(source, "ECCentral.Job");
                }
            ...
            catch (Exception ex)
            {
                try
                {
                    using (EventLog log = new EventLog("Application", ".", "ECCentral.Job.Utility"))
                    {
        ...
        private static string WriteLog(LogEntry log, List<ILogEmitter> logEmitterList)
        {
            ...
            try
            {
                ...
                foreach (var logEmitter in logEmitterList)
                {
                    ...
                    {
                        string message = string.Format("Write log failed.\r\n\r\n Error Info: {0}. \r\n\r\n Log Type: {1}. \r\n\r\n Log Info: {2}", ex.ToString(), logEmitter.GetType().AssemblyQualifiedName, log.ToXml());
                        Logger.WriteEventLog("ECCentral.Job.Utility_LoggingComponent", message, EventLogEntryType.Error);
                    }
                }
            }
            catch(Exception ex)
            {
                string message = string.Format("Write log failed.\r\n\r\n Error Info: {0}. \r\n\r\n Log Info: {1}", ex.ToString(), log.ToXml());
                Logger.WriteEventLog("ECCentral.Job.Utility_LoggingComponent", message, EventLogEntryType.Error);
            }
