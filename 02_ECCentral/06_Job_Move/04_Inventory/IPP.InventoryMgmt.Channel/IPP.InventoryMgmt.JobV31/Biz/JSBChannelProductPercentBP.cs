...
{
    public class JSBChannelProductPercentBP : ChannelInventoryBaseBP
    {
        public JSBChannelProductPercentBP(JobContext context) : base(context) { }

        protected override int GetRecordsCount()
        {
            return ChannelInventoryDA.GetPercentListCountByChannelSysNo(500);
        }

        protected override List<ChannelProductEntity> GetChannelProductList(int page, int pageSize)
        {
            List<ChannelProductPercentEntity> list = ChannelInventoryDA.GetAllPercentListByChannelSysNo(page, pageSize,500);
