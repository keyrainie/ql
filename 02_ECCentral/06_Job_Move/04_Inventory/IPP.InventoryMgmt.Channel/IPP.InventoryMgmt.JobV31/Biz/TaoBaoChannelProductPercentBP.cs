...
{
    public class TaoBaoChannelProductPercentBP : ChannelInventoryBaseBP
    {
        public TaoBaoChannelProductPercentBP(JobContext context) : base(context) { }

        protected override int GetRecordsCount()
        {
            return ChannelInventoryDA.GetPercentListCountByChannelSysNo(200);
        }

        protected override List<ChannelProductEntity> GetChannelProductList(int page, int pageSize)
        {
            List<ChannelProductPercentEntity> list = ChannelInventoryDA.GetAllPercentListByChannelSysNo(page, pageSize, 200);
